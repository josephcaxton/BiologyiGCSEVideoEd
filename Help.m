...
-(IBAction)ReportProblem:(id)sender{
	...
	if ([MFMailComposeViewController canSendMail]) {
        ...
        if([LCNOAuth isEqualToString:@"contactsupport"]){
            ...
            [SendMailcontroller setSubject:[NSString stringWithFormat:@"%@ / * %@ Biology iGCSE video streaming iPad",DeviceID,DeviceIDWhenPurchase]];
        }
        else{

        [SendMailcontroller setSubject:[NSString stringWithFormat:@"%@ Biology iGCSE video streaming iPad",DeviceID]];
        }
        [SendMailcontroller setMessageBody:[NSString stringWithFormat:@"Add Message here "] isHTML:NO];
        [self presentViewController:SendMailcontroller animated:YES completion:nil];

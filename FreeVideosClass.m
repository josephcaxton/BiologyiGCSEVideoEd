...
- (void)viewDidLoad {
    ...
    NSString *queryFeed = [NSString stringWithFormat:@"%@/iosStreamv2/biology/BiologyvideoiGCSEEd.xml",domain];
    NSString *Dir = [appDelegate.applicationDocumentsDirectory stringByAppendingPathComponent:@"BiologyvideoiGCSEEd.xml"];
...
-(void)MyParser:(NSString *)FileLocation{
	...
	for(int idx = 0; idx < pointStrings.count - 1; idx++)
	{
        ...
        obj.ProductID = @"BiologyiGCSEEdiPad";
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    if(section == 0){
        ...
        if([filteredArrayofConfigObjects count] > 0 && FullSubscription == FALSE ){
            sectionHeader.text = @"iGCSE Biology Edexcel – Start today from only £1.49";
        }
        else if ([filteredArrayofConfigObjects count] > 0 && FullSubscription == TRUE ){
            sectionHeader.text = @"My Courses – iGCSE Biology Edexcel";
        }
        else{
            sectionHeader.text =@"";
        }
...
- (void)reviewPressed {
    ...
    NSString *str = @"https://userpub.itunes.apple.com/WebObjects/MZUserPublishing.woa/wa/addUserReview?id=827757583&type=Purple+Software"; 
